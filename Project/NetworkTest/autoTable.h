...
{
private:
	autoList<autoList<Element_t> *> *Table;
...
public:
	autoTable(Element_t (*createCellFunc)(void), void (*releaseCellFunc)(Element_t), int w = 1, int h = 1)
	{
		...
		this->Table = new autoList<autoList<Element_t> *>();
	...
	~autoTable()
	{
		while(this->Table->GetCount())
		{
			autoList<Element_t> *row = this->Table->UnaddElement();
	...
	{
		m_swap(this->Table, otherTable->Table, void *);
	...
	void Resize(int w, int h)
	{
		...
		while(this->Table->GetCount() < h)
		{
			this->Table->AddElement(new autoList<Element_t>());
		}
		for(int rowidx = 0; rowidx < h; rowidx++)
		{
			autoList<Element_t> *row = this->Table->GetElement(rowidx);
	...
	int GetWidth()
	{
		return this->Table->GetCount() ? this->Table->GetElement(0)->GetCount() : 0;
	}
	int GetHeight()
	{
	...
	Element_t *CellAt(int x, int y)
	{
		...
		return this->Table->GetElement(y)->ElementAt(x);
	}
